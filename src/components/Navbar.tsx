...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex h-[72px] w-full bg-[#161B22] text-white items-center px-[2%] xl:px-[6%] gap-6">
        <Link href="/" className="shrink-0">
          <Github className="h-[32px] w-[32px] hover:cursor-pointer" />
        </Link>
        <div className="flex gap-4">
          {menu.map((item) => (
            <div key={item.id} className="flex items-center group hover:cursor-pointer hover:text-white/80 shrink-0 gap-[3px]">
              {item.name}
              {item.isArrow && (
                <div className="pt-1 group-hover:pt-2 duration-300 ease-in-out text-white/60 group-hover:text-white/40">
